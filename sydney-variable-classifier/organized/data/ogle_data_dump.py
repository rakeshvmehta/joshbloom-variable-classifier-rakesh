...
if __name__ == "__main__": 
    ...
    parser.add_argument('--min_sample', default=5,
                        help="Minimum number of samples per object"
                        )
    parser.add_argument('--outname', default="data/Raw_data/ogle_data_5.pkl",
    ...
    lightcurves = []
    for t in types:
        data = np.genfromtxt('data/Raw_data/'+t+'.txt',names=True, dtype=None, skip_header=6, usecols=range(37), encoding=None)
    ...
    unique_label, count = np.unique([lc.label for lc in lightcurves], return_counts=True)
    use_label = unique_label[count >= args.min_sample]
    new_data = []
    for cls in use_label:
        class_data = [lc for lc in lightcurves if lc.label == cls]
