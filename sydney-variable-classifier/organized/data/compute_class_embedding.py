...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_list', type = str, default = None, help = 'Path to a file containing the IDs of the classes to compute embeddings for (as first words per line). If not given, all nodes in the hierarchy will be considered as target classes.')
...
Default: "unitsphere"''')
    ...
    parser.add_argument('--metric', type = int, default = 2, help = 'Pick metric 1 (B&D) or 2 (Tanimoto).') #Added
    ...
    if args.class_list is not None:
        ...
        unique_labels = [lbl for lbl in hierarchy.nodes]# if (lbl not in hierarchy.children) or (len(hierarchy.children[lbl]) == 0)] #Changed
        if not args.str_ids:
            unique_labels.sort() 
    ...
    for i in range(len(unique_labels)):
        for j in range(i + 1, len(unique_labels)):
            #print(unique_labels[i], unique_labels[j])
            if args.metric==1:
                sem_class_dist[i,j] = sem_class_dist[j,i] = hierarchy.lcs_height(unique_labels[i], unique_labels[j])
            elif args.metric==2:
                sem_class_dist[i,j] = sem_class_dist[j,i] = hierarchy.metric2(unique_labels[i], unique_labels[j])
    print(sem_class_dist)
