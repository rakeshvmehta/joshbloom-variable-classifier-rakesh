...
def data_generator(batch_size, dataset, train_size, partition=3, random_state=1):
    k = partition
    ...
    if k != None:
        ...
        for label in unique_label: 
            ...
            step = 1 if math.ceil(class_size/k)==2 else math.ceil(class_size/k)
            list_of_lists = [class_data[i:(i+math.ceil(class_size/k))] for i in range(0, class_size, step)]
            print(class_size, math.ceil(class_size/k), len(list_of_lists),k)
            for i in range(k):
                k_lists[i].extend(list_of_lists[i])
        train_sets, val_sets, test_sets = [],[],[]
        for i in range(1): #Can change to k to make multiple datasets that have mutually exclusive training groups
            ...
            max_sizes = [train_size, 999999999, 999999999]
