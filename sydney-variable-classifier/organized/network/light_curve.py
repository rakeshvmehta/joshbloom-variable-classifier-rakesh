#Code from https://github.com/kmzzhang/periodicnetwork
import numpy as np
...
class LightCurve():
    ...
    def copy(self):
        return LightCurve(self.times, self.measurements, self.errors, survey=self.survey, name=self.name, best_period=self.best_period, best_score=self.best_score, label=self.label, p=self.p, p_signif=self.p_signif, p_class=self.p_class, ss_resid=self.ss_resid)
    ...
    def period_fold(self, p=None):
        ...
        if p is None:
            p = self.p
        try:
            self.times = self.times % float(p)
        except:
            print(type(self.times), type(p))
            print(p)
            print(self.times)
