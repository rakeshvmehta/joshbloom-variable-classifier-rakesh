...
parser.add_argument('--cudnn_deterministic', action='store_true', default=True,#False,
                    help='')
parser.add_argument('--min_sample', type=int, default=0,#0,
...
parser.add_argument('--maxsize', type=int, default=0,
                    help='Which augmented dataset to use based on max size')
parser.add_argument('--filepath', type=str, default=None,
                    help='Data directory in data')
parser.add_argument('--datagen', type=int, default=1,
                    help='Use updated data generators, 1 or 0')
...
if args.classification=='none' and (args.network == 'resnet' or args.network == 'iresnet'):
    save_name = '{}-{}-K{}-D{}-NL{}-H{}-MH{}-L{}-V{}-{}-LR{}-CLIP{}-DROP{}-TP{}-MS{}'.format(args.filename[:-4],
                                                                              args.network,
                                                                              args.kernel,
                                                                              args.depth,
                                                                              args.n_layer,
                                                                              ...
                                                                              int(args.two_phase), args.maxsize)
elif args.network == 'resnet' or args.network == 'iresnet':
    save_name = '{}-{}-K{}-D{}-NL{}-H{}-MH{}-L{}-V{}-{}-LR{}-CLIP{}-DROP{}-TP{}-CL{}-MS{}'.format(args.filename[:-4],
                                                                              args.network,
                                                                              args.kernel,
                                                                              args.depth,
                                                                              args.n_layer,
                                                                              ...
                                                                              max(args.dropout, args.dropout_classifier),
                                                                              int(args.two_phase),
                                                                              args.classification, args.maxsize)
else:
    save_name = '{}-{}-K{}-D{}-H{}-L{}-V{}-{}-LR{}-CLIP{}-DROP{}-TP{}-CL{}-MS{}'.format(args.filename[:-4],
                                                                              args.network,
                                                                              args.kernel,
                                                                              args.depth,
                                                                              args.hidden,
...
from model.resnet import Classifier as resnet_original
...
data = joblib.load('data/{}'.format(args.filename)) if args.filepath == None else joblib.load('data/{}/{}'.format(args.filepath, args.filename))
if args.maxsize != 0 and args.filepath != None: 
    filename = args.filename.split("_train")[0] if args.filepath == None else '{}/{}'.format(args.filepath, args.filename.split("_train")[0])
    test_data = joblib.load('data/{}_test.pkl'.format(filename))
    val_data = joblib.load('data/{}_val.pkl'.format(filename))
...
print("Use label:",use_label, unique_label, count)
...
def get_network(n_classes, embedding, hidden=args.hidden, max_hidden=args.max_hidden, depth=args.depth, kernel=args.kernel, dropout_classifier=args.dropout_classifier, n_layer=args.n_layer, hidden_classifier=args.hidden_classifier):
    if args.network == 'itcn':
        ...
        clf = resnet(n_inputs, n_classes, depth=depth, nlayer=n_layer, kernel_size=kernel,
                     ...
                     aux=3, dropout_classifier=dropout_classifier, hidden=hidden_classifier,
...
def get_aux(split):
    ...
    return split, X_list, periods, label, x, means, scales, aux, scales_all
    
    
def train_helper(param):
    global map_loc
    train_data, val_data, test_data, name = param
    
    split = [chunk for i in range(len(train_data)) for chunk in train_data[i].split(args.L, args.L) if train_data[i].label is not None]
    for lc in split:
        lc.period_fold()
    split, X_list, periods, label, x, means, scales, aux, scales_all = get_aux(split)
    
    val_split = [chunk for i in range(len(val_data)) for chunk in val_data[i].split(args.L, args.L) if val_data[i].label is not None]
    for lc in val_split:
        lc.period_fold()
    val_split, val_X_list, val_periods, val_label, val_x, val_means, val_scales, val_aux, val_scales_all = get_aux(val_split)
    ...
    rnd_idxs = np.random.choice(len(label), len(label), replace=False)
    x, aux, label = x[rnd_idxs], aux[rnd_idxs], label[rnd_idxs]
    rnd_idxs = np.random.choice(len(val_label), len(val_label), replace=False)
    val_x, val_aux, val_label = val_x[rnd_idxs], val_aux[rnd_idxs], val_label[rnd_idxs]

    train_dset = MyDataset(x, aux, label)
    val_dset = MyDataset(val_x, val_aux, val_label)
    train_loader = DataLoader(train_dset, batch_size=args.train_batch, shuffle=False, drop_last=True)#, sampler = sampler)
    val_loader = DataLoader(val_dset, batch_size=128, shuffle=False, drop_last=False)

    split = [chunk for i in range(len(test_data)) for chunk in test_data[i].split(args.L, args.L)]
    for lc in split:
        lc.period_fold()
    test_split, test_X_list, test_periods, test_label, test_x, test_means, test_scales, test_aux, test_scales_all = get_aux(split)
    rnd_idxs = np.random.choice(len(test_label), len(test_label), replace=False)
    x, aux, label = test_x[rnd_idxs], test_aux[rnd_idxs], test_label[rnd_idxs]
    test_dset = MyDataset(test_x, test_aux, test_label)
    ...
    if not args.no_log:
        ...
        print("Project and name: ",args.project_name, name)
    ...
    if args.optimize: 
        ...
        mdl = get_network(n_classes, embedding, hidden, max_hidden, depth, kernel, dropout_classifier, n_layer, hidden_classifier)
    else:
        hidden = args.hidden
    ...
    if not args.test:
        ...
        train(mdl, optimizer, train_loader, val_loader, test_loader, args.max_epoch, embedding, embed_labels, 
              linear_labels, label2int, convert_ind, args.classification, convert_emb2ind, hierarchy, print_every=args.print_every, 
    ...
    if True:#args.classification != 'none':
        ...
        for j, length in enumerate(lengths):
            ...
            for i, d in enumerate(test_loader):
                ...
                if args.classification=='none':
                    ...
                    predicted_embed.extend(logprob1.numpy())
            ...
            predicted_embed = np.array(predicted_embed)
            preds.append(predictions)
            truth.append(ground_truths)
            probs.append(probabilities)
            embeds_list.append(predicted_embed)
...
if __name__ == '__main__':
    ...
    np.random.seed(args.seed)
    if args.datagen: 
        train_loader, val_loader, test_loader = data_generator(args.train_batch, args.filename, args.maxsize, k=args.K)
        for i in range(args.K):
            jobs.append((train_loader[i], val_loader[i], test_loader[i], '{}/{}-{}'.format(args.path, save_name, i)))
    else: 
        for i in range(args.K):
            if args.K == 1:
                i = args.pseed
            ...
            if args.maxsize != 0: 
                new_frac_train = args.frac_valid*args.frac_train + args.maxsize/len(all_labels)
                trains, tests = train_test_split(all_labels, train_size=new_frac_train, random_state=i)
    ...
    if args.optimize: 
        results = []
        results.append(train_helper(jobs[0]))
        
    elif args.ngpu <= 1 and args.njob == 1:
        results = []
        for j in jobs:
    ...
    np.save('{}/{}{}-results-embed.npy'.format(args.path, save_name, args.note), results_embed)
