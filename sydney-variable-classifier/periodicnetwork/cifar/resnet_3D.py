...
class MyMaxPool1d(nn.Module):
    def __init__(self, window, stride=1, min_length=-1, padding='zero'):
        ...
        self.window = window
        self.maxpool = nn.MaxPool2d(window, stride=stride)
...
class Classifier(nn.Module):
    def __init__(self, num_inputs, num_class, depth=9, nlayer=2, kernel_size=3, hidden_conv=32, max_hidden=256,
        ...
        self.linear1 = nn.Conv2d(h + aux, hidden, 1)
        self.linear2 = nn.Conv2d(hidden, num_outputs, 1)
        self.dropout = nn.Dropout(dropout_classifier)
        self.aux = aux
        if aux > 0:
            self.linear = nn.Sequential(self.linear1, self.dropout, nn.ReLU(), self.linear2)
        else:
            self.linear = nn.Conv2d(h, num_outputs, 1)
    ...
    def forward(self, x, aux=None):
        # N D L
        f = self.a(x)
        #print("SHAPE A:",f.size())
        f = self.conv(f)
        #print("SHAPE all res blocks:", f.size())
        feature = self.conv2(f)
        #print("SHAPE feature:", feature.size())
        self.cache = feature
        if self.aux > 0:
        ...
        logprob = logprob.contiguous()
        logprob = logprob.view(-1, 121)
    ...
    def __init__(self, in_ch, out_ch, k, dropout=0, padding='zero'):
        ...
        if padding == 'cyclic':
            net = [wrap(k-1, mode=padding),
                   weight_norm(nn.Conv2d(in_ch, out_ch, k)),
                   nn.ReLU(),
                   nn.Dropout(dropout),
                   wrap(k-1, mode=padding),
                   weight_norm(nn.Conv2d(out_ch, out_ch, k)),
                   nn.ReLU(),
                   nn.Dropout(dropout)]
        if padding == 'zero':
            net = [weight_norm(nn.Conv2d(in_ch, out_ch, k, padding=int(k/2))),
                   nn.ReLU(),
                   nn.Dropout(dropout),
                   weight_norm(nn.Conv2d(out_ch, out_ch, k, padding=int(k/2))),
                   nn.ReLU(),
                   nn.Dropout(dropout)]
