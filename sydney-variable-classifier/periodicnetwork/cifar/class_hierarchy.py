...
class ClassHierarchy(object):
    ...
    def __init__(self, parents, children):
        ...
        self._compute_distances() #Added
        self.max_height = max(self.heights.values())
        self._compute_root() #Added
    ...
    def _compute_distances(self): #Added
        """ Computes the distances of all nodes in the hierarchy. """
        
        def distance(id, id2, pathlength):
            
            if id not in self.distances:
                self.distances[id] = {}
            self.distances[id][id] = 0
            if id2 in self.children: 
                for child in self.children[id2]: 
                    if child not in self.distances:
                        self.distances[child] = {}
                    self.distances[id][child] = self.distances[child][id] = pathlength
                    distance(id, child, pathlength+1)
    ...
    def _compute_root(self): #Added
        for nd in self.children.keys():
            if nd not in self.parents.keys():
                self.root = nd 
    ...
    def metric2(self, a, b): #Added
        """ Computes 1-Tanimoto distance
        ...
        """
        lcs = self.lcs(a,b)
        pathlength = self.distances[lcs][a] + self.distances[lcs][b] 
        denom = 1 + self.distances[lcs][self.root] + pathlength
        return pathlength / denom
    
    def pathlength(self, a, b): #Added
        """ Computes pathlength between 2 nodes
        """
        lcs = self.lcs(a,b)
        return self.distances[lcs][a] + self.distances[lcs][b] 
