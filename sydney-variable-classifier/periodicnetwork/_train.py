...
def train(model, optimizer, train_loader, val_loader, test_loader, n_epoch, embeddings, embed_labels, 
          linear_labels, convert, convert_ind, classification, convert_emb2ind, hierarchy, map_loc,  eval_after=1e5, patience=10, min_lr=0.00001,
    ...
    for epoch in tqdm(range(n_epoch), disable=not use_tqdm):
        ...
        for i, d in tqdm(enumerate(train_loader), disable=not use_tqdm):
            x, aux, y = d
            x = x.type(dtype).to(map_loc)
            aux = aux.type(dtype).to(map_loc) 
            y = y.type(dint).to(map_loc)
           ...
           #print("LOSS:",loss)
            ...
            if classification=='hidden':
                ...
                #print("acc:",(np.array(map(convert_emb2ind, np.argmax(softmax(cosdistance(embeddings,logprob1)).detach().cpu(), axis=1))) == np.array(y.numpy())).mean())
                #predictions.extend(list(map(find_c_label, softmax(cosdistance(embeddings,logprob1)).detach().cpu())))
            ground_truths.extend(list(y.cpu().numpy())) 
