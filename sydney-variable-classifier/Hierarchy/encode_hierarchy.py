...
# Additions/Changes noted by #Added/#Changed
...
def read_hierarchy(filename):
    ...
    with open(filename) as f:
        for li, l in enumerate(f, start = 0):
            l = l.strip()
            if l != '':
...
if __name__ == '__main__':
    ...
    parser.add_argument('hi_file', type=str, default='variability_hierarchy.txt', help='The input file specifying the hierarchy in indented tree format.')
    parser.add_argument('m_file', type=str, default='variability_meta.txt', help='Path to the meta pickle file from CIFAR-100.')
    parser.add_argument('--out', type=str, default='variability.parent-child.txt', help='Output file containing parent-child tuples.')
    ...
    with open(args.m_file, 'rb') as meta_text:
        #meta = pickle.load(meta_pickle, encoding = 'bytes')
        meta = [lbl.decode()[:-1] for lbl in meta_text] #Changed
    
    hierarchy = read_hierarchy(args.hi_file)
    if args.plot is not None:
        plot_hierarchy(hierarchy, args.plot)
    hierarchy, node_names = encode_class_names(hierarchy, meta) #Changed
    #[lbl.decode() for lbl in meta[b'fine_label_names']])
